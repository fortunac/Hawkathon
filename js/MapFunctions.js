function USStateCountiesData(Targetsvg, stateID) {
    ...
    d3.json("js/us.json", function (error, us) {
     ...
     .enter().append("path")
       ...
       .on("click", clicked);
        ...
        d3.json("/Hospitals.json", function (error, providers) {
            ...
            var StateData = _.filter(providers, function (d) { return (parseInt(d.State_Fips) === parseInt(stateID)) });
           
            g.selectAll("hosptital")
                    .data(StateData)
                    .enter()
                    .append("circle").attr("class", "hosptital").attr("cx", function (d) {
     ...
     .on("mouseout", function (d) {
        ...
        d3.json("/Pharmacy.json", function (error, Pharmacys) {
            if (error) console.log(error);
            var Pharmacy = _.filter(Pharmacys, function (d) { return ((Math.floor(d.Fips / 1000) === parseInt(stateID))) })
            ...
            g.selectAll("cities")
                    ...
                    .append("circle").attr("class", "cities").attr("cx", function (d) {
                        //    console.log(d.Longitude + ", " + d.Latitude);
                        // console.lolog(projection([parseFloat(d.Longitude), parseFloat(d.Latitude)])[0]);
     ...
     .on("mouseout", function (d) {
        ...
        if (stateID === 25) {
            d3.json("MA2.json", function (error, HCPs) {
               
                if (error) console.log(error);
                 console.log(HCPs);
                //var HCP = _.filter(HCPs, function (d) { return ((Math.floor(d.Fips / 1000) === parseInt(stateID))) })
                console.log(HCPs);
                g.selectAll("providers")
                        .data(HCPs)
                        .enter()
                        .append("circle").attr("class", "providers").attr("cx", function (d) {
                            //    console.log(d.Longitude + ", " + d.Latitude);
                            // console.lolog(projection([parseFloat(d.Longitude), parseFloat(d.Latitude)])[0]);
        ...
        function clicked(d) {
            ...
            active = d3.select(this).classed("activestateid", true);
...
function USMap(Targetsvg, StateSVG) {
    ...
    function clicked(d) {
        ...
        USStateCountiesData(StateSVG, d.id);
        if (active.node() === this) return reset();
        active.classed("active", false);
        active = d3.select(this).classed("active", true);
